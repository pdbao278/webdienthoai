...
describe('Admin Order API', () => {
  ...
  beforeAll(async () => {
    const admin = await prisma.user.create({
      data: { email: `admin_${Date.now()}@test.com`, passwordHash: 'pwd', role: Role.ADMIN }
    });
    adminToken = jwt.sign({ id: admin.id, email: admin.email, role: admin.role }, JWT_SECRET);

    const customer = await prisma.user.create({
      data: { email: `cust_${Date.now()}@test.com`, passwordHash: 'pwd', role: Role.CUSTOMER }
    });
  ...
  describe('POST /api/admin/orders/scan', () => {
    it('completes order via QR scan', async () => {
      const res = await request(app)
