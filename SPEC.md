...
| US-02 | Là Khách vãng lai, tôi có thể xem chi tiết điện thoại với thông số kỹ thuật & thư viện ảnh | Trang sản phẩm hiển thị hình ảnh, bảng thông số kỹ thuật, giá, tình trạng kho hàng, khuyến mãi. |
...
| 3 | Dữ liệu mẫu (Seed) | **~10 mẫu cho mỗi hãng** (~130 sản phẩm cho 13 hãng) | Sẽ bổ sung thêm sau. Kịch bản seed phải hỗ trợ khả năng thêm sản phẩm tăng dần. |
...
| `products` | Danh mục điện thoại | id, slug, hang, san_pham, phan_khuc, gia_goc, gia_ban, ton_kho, mo_ta, + tất cả các trường cấu hình từ PRD |
| `product_media` | Ảnh/video sản phẩm | id, product_id, url, public_id, loai (image/video), thu_tu, is_thumbnail |
| `cart_items` | Giỏ hàng của người dùng | id, user_id, product_id, so_luong |
...
| `order_items` | Chi tiết mặt hàng đơn hàng | id, order_id, product_id, so_luong, don_gia |
...
|---|---|---|
| **M1: Nền tảng & Danh mục** | Tuần 1-2 | Cấu hình monorepo, sơ đồ database + seed dữ liệu mẫu, Trang chủ, Trang danh sách + bộ lọc, Trang chi tiết sản phẩm, Xác thực người dùng (đăng ký/đăng nhập/xác thực email) |
| **M2: Luồng mua sắm** | Tuần 3-4 | Giỏ hàng lưu DB, Luồng thanh toán Click & Collect, Quản lý đơn hàng cá nhân, Gửi email thông báo đơn hàng |
| **M3: Admin & Vận hành** | Tuần 5-6 | Dashboard quản trị, CRUD sản phẩm, Xử lý đơn hàng & quét QR, Quản lý Voucher, Đánh giá, So sánh sản phẩm, Báo cáo doanh thu |
| **M4: Hoàn thiện & Phát hành** | Tuần 7 | Tối ưu hóa SEO, Tối ưu hiệu năng tải trang, Sửa lỗi, Triển khai chatbot tối giản, Deploy ứng dụng lên Vercel/Railway |

---

*Phiên bản đặc tả: v1.0 | Ngày tạo: 24/06/2026 | Nguồn: PRD.md v4.0 + DESIGN.md*
