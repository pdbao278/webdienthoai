...
- **Giao diện Admin (CRUD):** Bổ sung một giao diện riêng biệt trong Admin Dashboard để quản trị Flash Sale.
- **Tính năng Admin:** Cho phép Admin chọn sản phẩm, phân loại cụ thể theo biến thể (mẫu máy, dung lượng, màu sắc). Cho phép thiết lập giới hạn số lượng bán (`so_luong_khuyen_mai`) và đặt giá khuyến mãi (`gia_flash_sale`).
