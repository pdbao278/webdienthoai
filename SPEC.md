...
| US-11 | Là Khách vãng lai, tôi có thể so sánh sản phẩm nhanh chóng | Xem gợi ý sản phẩm tương đương (cùng giá, cấu hình) qua thuật toán cục bộ và thêm vào khay so sánh (tối đa 4 sản phẩm). |
...
## Mở rộng: Tính năng Flash Sale (Khung giờ) - [NEW Tính Năng Mới]
> **Ghi chú:** Tính năng này được bổ sung sau cùng để tăng tính kích thích mua sắm.

### 1. Mục tiêu & Tính năng cốt lõi
- **Mục tiêu:** Tính năng Giảm giá chớp nhoáng (Flash Sale) trên Trang chủ theo 4 mốc thời gian cố định mỗi ngày: **00:00, 09:00, 12:00, và 18:00**.
- **Tính năng & Vị trí:** Đặt giữa 2 khu vực Banner. Tự động xác định khung giờ để đếm ngược (Giờ : Phút : Giây) đến khi kết thúc đợt Sale hiện tại. Hiển thị Grid các sản phẩm có tag giảm giá ưu đãi.
- **Yêu cầu Kỹ thuật:** Tránh lỗi Hydration Mismatch bằng cách tính toán thời gian chạy sau mount (useEffect). Sử dụng Next.js, Tailwind v4 và các màu gradient nổi bật để thu hút sự chú ý.

### 2. Quản trị & Phân quyền (Admin CRUD)
- **Quyền hạn (RBAC):** CHỈ CÓ tài khoản quyền `ADMIN` mới được phép thao tác và thiết lập Flash Sale. (Khách hàng và Manager không có quyền).
- **Giao diện Admin (CRUD):** Giao diện quản lý Flash Sale được thiết kế với Select Box cố định cho thời gian: chọn Ngày (Hôm nay, Ngày mai, Mốt) và Khung giờ cố định (00:00, 09:00, 12:00, 15:00, 18:00). Hệ thống tự tính thời gian `batDau` và `ketThuc` thay vì để Admin chọn datetime tự do, đảm bảo các slot thời gian chuẩn xác.
- **API Timeline & Frontend Logic:** 
  - API Flash Sale `/api/flash-sales/today` trả về mảng các chiến dịch trong ngày. 
  - Frontend LUÔN hiển thị 5 tab cố định (00:00, 09:00, 12:00, 15:00, 18:00) cho thanh Timeline, sau đó merge data từ API vào. Việc này giúp UI luôn nhất quán (kể cả khi rỗng).
  - Khung giờ đang diễn ra được auto-select và auto-scroll tới, có Countdown Timer đếm ngược.
  - Các slot rỗng sẽ hiển thị "Chưa có sản phẩm flash sale..".
  - Slot chờ mở bán (Sắp diễn ra) sẽ có UI ẩn giá (ẩn các số phía sau `1?.???.??? ₫`) và thanh progress ở 0% ("Mở bán X suất").
- **Tự động hóa giá:** Giá của sản phẩm tự động nhảy về giá khuyến mãi khi nằm trong khung giờ quy định, và **tự nhảy về giá gốc** ngay khi Flash Sale kết thúc (quản lý bằng logic Database/Backend thay vì sửa giá tay).
- **Giới hạn số lượng mua (Per User):** Mỗi khách hàng chỉ được mua tối đa 1 sản phẩm Flash Sale cho mỗi biến thể. Nếu mua số lượng lớn hơn 1, hệ thống tính tiền theo công thức: `1 * giá_flash_sale + (số_lượng_mua - 1) * giá_gốc`.
---

## Mở rộng: Tính năng So Sánh & Gợi ý Thông Minh (Smart Quick Compare) - FR-08
> **Ghi chú:** Tính năng này giúp tối ưu hóa trải nghiệm lựa chọn của người dùng thông qua việc gợi ý thông minh các sản phẩm đối thủ hoặc tương đương.

### 1. Mục tiêu & Tính năng cốt lõi
- **Mục tiêu:** Giúp người dùng so sánh trực quan thông số kỹ thuật của tối đa 4 điện thoại cùng lúc, đồng thời tự động đề xuất các thiết bị thay thế phù hợp để tiết kiệm thời gian tìm kiếm.
- **Hiển thị:** Bảng so sánh đặt tại trang danh sách hoặc cuối trang chi tiết sản phẩm. Cấu hình được đối chiếu theo từng hàng ngang (Ví dụ: Chipset, RAM, Pin, Giá).

### 2. Thuật toán Gợi ý Cục bộ (Local Heuristics)
- **Kiến trúc:** Thay vì gọi API AI bên ngoài (gây độ trễ và tốn chi phí), module gợi ý chạy hoàn toàn nội bộ trên Backend bằng các thuật toán Rule-based hoặc thư viện khoảng cách (ví dụ: `ml-knn`, `content-based-recommender`).
- **Logic Đề xuất:** Hệ thống tự động truy vấn Database để lọc ra danh sách các máy có đặc tính tương tự:
  - Cùng tầm giá (mức dao động ± 15%).
  - Có cấu hình bộ nhớ tương đương (RAM/ROM).
  - Ưu tiên hiển thị các sản phẩm cùng hãng, hoặc cùng phân khúc (Flagship, Tầm trung, ...).

### 3. Tìm kiếm Mở rộng
- Ngoài danh sách gợi ý tự động, module so sánh có tích hợp thanh tìm kiếm thông minh (Autocomplete dropdown) để người dùng tự do nhập tên model máy và thêm vào khay so sánh hiện tại.

---

*Phiên bản đặc tả: v1.2 | Ngày cập nhật: 01/07/2026 | Nguồn: PRD.md v4.5 + DESIGN.md*
